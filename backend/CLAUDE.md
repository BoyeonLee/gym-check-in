...
│   │   └── admins.go
│   ├── # domain/           # MVP에서는 미생성 — 도메인 로직(권한 체크, 회원권
│   │                       # 라이프사이클, 체크인 트랜잭션)은 internal/http 핸들러가
│   │                       # WithTx로 repo를 직접 조립한다. 복잡도 증가 시 추출.
│   │                       # 자세한 배경: docs/ARCHITECTURE.md "패턴" 섹션.
...
## 규칙
### 일반
- **CRITICAL**: SQL은 오직 `internal/repo`에만 존재한다. 핸들러(MVP에선 도메인 서비스 역할도 흡수)는 리포지토리 함수를 통해서만 DB에 접근한다 — `WithTx`로 트랜잭션을 묶고 repo를 조립.
...
- **CORS**: 미들웨어가 모든 응답에 다음을 포함한다. preflight `OPTIONS` 요청에는 본문 없이 204 No Content + 아래 헤더만 응답(빈 응답에는 204가 의미상 정확).
...
- `POST /api/admin/refresh`: refresh 토큰을 검증 → DB에서 admin row 재조회 → 새 access 토큰 발급. `revoked_refresh_tokens` 테이블에서 `jti` 조회되면 401. 응답 body에 `access_token`/`expires_in` 외에 admin row의 최신 `must_change_password`/`role`/`branch_id`/`username`을 함께 반환해 프론트가 access_token 디코딩 없이도 권한 변동(role 토글·branch 이동·임시비번 강제 변경)을 즉시 반영할 수 있게 한다.
...
- `POST /api/admin/password`는 항상 **현재 비밀번호 재입력**을 검증한다. bcrypt 비교 실패 시 401, 성공 시 새 해시로 갱신 + `must_change_password=false` + `password_updated_at=now()` + `temp_password_expires_at=NULL` 세팅. **refresh·access 무효화는 unified `password_updated_at` 메커니즘으로 처리**한다 — 즉 `revoked_refresh_tokens`에 명시 INSERT하는 대신, 다음 refresh 호출 시 admin row 재조회 시점에 Auth 미들웨어의 `iat < password_updated_at` 검증으로 stale refresh가 401로 차단되고, stale access도 같은 검증으로 다음 요청에서 401된다(이중 무효화 비용 없이 같은 효과).
- **비번 변경 후 stale access 즉시 차단**: 비번 변경 응답 후 다른 디바이스에 살아있는 access(만료까지 최대 30분)는 Auth 미들웨어의 `iat < password_updated_at` 검증으로 다음 요청에서 401 처리된다(같은 메커니즘이 refresh에도 적용 — admin row 재조회 시점에 차단). 호출한 본인 디바이스는 응답 처리 후 새 토큰을 받기 위해 재로그인하거나, 클라이언트가 비번 변경 폼에서 자동 로그아웃 → 재로그인 유도.
...
- `PATCH /api/admins/:id`는 전역 토큰만 통과. 변경 가능 필드: `username`, `role`, `branch_id`. 검증: role/branch_id 조합(`global`은 branch_id NULL 강제, `branch`는 branch_id 필수), username 유니크. `password_hash`·`must_change_password`·`failed_login_count`·`locked_until` 등은 PATCH로 변경 불가(별도 엔드포인트 사용). `branch_id` 변경 시 unified `password_updated_at` bump으로 access·refresh 모두 다음 요청에서 무효화(권한 변동을 즉시 반영).
- `DELETE /api/admins/:id`는 전역 토큰만 통과 + 호출자 본인 계정은 삭제 불가(409 `CANNOT_DELETE_SELF`). soft delete + `password_updated_at` bump으로 access·refresh 모두 무효화.
...
### 계층
- **단위 테스트**(`internal/auth`, `internal/util`, `internal/cache`, `internal/idempotency`, `internal/apperr`): 외부 의존성 없는 순수 함수. 만료일 계산·비번 강도·cursor 인코딩·KST 날짜·임시 비번 생성기·JWT claim·LRU 캐시·Idempotency UUIDv4·DB 에러 매핑 등. (MVP에선 `internal/domain`이 없고 도메인 로직은 핸들러에 흡수됨.)
