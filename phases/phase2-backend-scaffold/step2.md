...
- `docs/API.md` — 에러 코드 카탈로그(429 `RATE_LIMIT_EXCEEDED`, 400 `BODY_TOO_LARGE`, 500 `INTERNAL`)
...
- 초과 시 429 + `{"error":{"code":"RATE_LIMIT_EXCEEDED","message":"..."}}` + `Retry-After: <초>` 헤더.
...
- **응답 일관성**: 모든 에러 응답은 apperr 형식. recovery는 panic을 500 INTERNAL로, body limit은 400 BODY_TOO_LARGE로, rate limit은 429 RATE_LIMIT_EXCEEDED로.
